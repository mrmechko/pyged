...
class GedBase(object):
    ...
    def calculate_costs(self):
        ...
        if self.greedy:
            ...
            self.row_ind, self.col_ind = optimize.linear_sum_assignment(cost_matrix)
            
            if self.verbose:
                for row, column in (self.row_ind, self.col_ind):
    ...
    def edit_path(self):
        ...
        for i, j in zip(self.row_ind, self.col_ind):
            edits.append(self.decode_edit(i,j))
        return edits

    def decode_edit(self, i, j):
        if i < self.N and j < self.M:
            return "sub", self.g1.nodes()[i], self.g2.nodes()[j]
        elif self.N < i < self.N + self.M and j < self.M:
            return "ins", self.g2.nodes()[j] # insert a node from target
        elif i < self.N and self.M < j < self.N + self.M:
            return "del", self.g1.nodes()[i] # delete a node from source
